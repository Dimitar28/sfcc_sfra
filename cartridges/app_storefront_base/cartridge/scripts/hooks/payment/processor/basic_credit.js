...
function Authorize(orderNumber, paymentInstrument, paymentProcessor) {
    ...
    var fieldErrors = {};
    var error = false;
    var email = customer.profile.getEmail();
    var bmEmail = dw.system.Site.getCurrent().getCustomPreferenceValue("businessManagerEmail");
    var warnLogger = Logger.getLogger("warn");
    try {
        if (email !== bmEmail) {
            Transaction.wrap(function () {
            ...
            error = true;
            warnLogger.warn("Email mismatch: Customer email " + email + " does not match Business Manager email " + bmEmail + "");
        }
    } catch (e) {
        error = true;
        serverErrors.push(
